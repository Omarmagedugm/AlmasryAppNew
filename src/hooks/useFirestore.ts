        ...
        getDoc(doc(db, 'users', currentUser.uid))
          .then((snap) => {
            if (snap.exists()) {
              ...
              const isAdmin = currentUser.email?.toLowerCase() === 'copyrightofficialco@gmail.com' || 
                              currentUser.email?.toLowerCase() === 'omarmagedugm@gmail.com' ||
                              currentUser.email?.toLowerCase() === 'itthadalexchannel2@gmail.com' ||
                              currentUser.email?.toLowerCase() === 'itthadalexchannel2@masry.club' ||
                              currentUser.email?.toLowerCase()?.startsWith('itthadalexchannel2@');
              const userData = {
                uid: currentUser.uid,
                name: currentUser.displayName || 'مشجع مصراوي',
                email: currentUser.email || '',
                role: (isAdmin ? 'admin' : 'user') as 'user' | 'admin',
                username: currentUser.email?.split('@')[0].toLowerCase() || `user_${currentUser.uid.substring(0, 5)}`,
                tier: 'new',
